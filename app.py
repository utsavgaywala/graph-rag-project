...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    .main-title { font-size: 2rem; font-weight: 700; background: linear-gradient(135deg, #7F77DD, #1D9E75, #EF9F27); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-align: center; padding: 0.5rem 0; }
    .subtitle { text-align: center; color: #666; font-size: 0.8rem; margin-bottom: 0.5rem; letter-spacing: 1px; }
    .stat-card { background: linear-gradient(135deg, rgba(127,119,221,0.15), rgba(29,158,117,0.15)); border: 1px solid rgba(127,119,221,0.3); border-radius: 12px; padding: 0.8rem; text-align: center; }
    .stat-number { font-size: 1.8rem; font-weight: 700; color: #7F77DD; }
    .stat-label { font-size: 0.7rem; color: #888; text-transform: uppercase; letter-spacing: 1px; }
    .graph-connection { background: rgba(29,158,117,0.1); border-left: 3px solid #1D9E75; border-radius: 0 8px 8px 0; padding: 0.3rem 0.8rem; margin: 0.2rem 0; font-family: monospace; font-size: 0.75rem; color: #1D9E75; }
    .pipeline-step { background: rgba(239,159,39,0.1); border: 1px solid rgba(239,159,39,0.3); border-radius: 8px; padding: 0.5rem; font-size: 0.8rem; color: #EF9F27; }
    .feature-item { background: rgba(127,119,221,0.1); border-radius: 6px; padding: 0.3rem 0.6rem; margin: 0.2rem 0; font-size: 0.8rem; color: #ccc; }
    .sidebar-title { font-size: 1rem; font-weight: 600; color: #7F77DD; }
    .main .block-container { padding-top: 1rem !important; padding-bottom: 120px !important; max-width: 100% !important; }
    section[data-testid="stBottom"] { background: linear-gradient(0deg, #0d0d1f 60%, transparent) !important; padding: 1rem 2rem 1.5rem 2rem !important; backdrop-filter: blur(20px) !important; }
    div[data-testid="stChatInput"] { background: rgba(255,255,255,0.05) !important; border: none !important; border-radius: 50px !important; padding: 0.3rem 1rem !important; box-shadow: none !important; }
    div[data-testid="stChatInput"] textarea { background: transparent !important; color: white !important; font-size: 0.95rem !important; border: none !important; outline: none !important; padding: 0.5rem 0 !important; }
    div[data-testid="stChatInput"] textarea::placeholder { color: rgba(127,119,221,0.6) !important; font-style: italic !important; }
    div[data-testid="stChatInput"] button { background: linear-gradient(135deg, #7F77DD, #1D9E75) !important; border-radius: 50% !important; border: none !important; width: 38px !important; height: 38px !important; }
    div[data-testid="stChatMessage"] { background: rgba(255,255,255,0.03) !important; border: 1px solid rgba(127,119,221,0.1) !important; border-radius: 16px !important; padding: 1rem !important; margin: 0.5rem 0 !important; }
    div[data-testid="stExpander"] { background: rgba(255,255,255,0.02) !important; border: 1px solid rgba(127,119,221,0.2) !important; border-radius: 12px !important; }
    .verdict-grounded { background: rgba(29,158,117,0.15); border: 1px solid rgba(29,158,117,0.4); border-radius: 8px; padding: 0.4rem 0.8rem; font-size: 0.8rem; color: #1D9E75; display: inline-block; margin-top: 0.5rem; }
    .verdict-hallucinated { background: rgba(216,90,48,0.15); border: 1px solid rgba(216,90,48,0.4); border-radius: 8px; padding: 0.4rem 0.8rem; font-size: 0.8rem; color: #D85A30; display: inline-block; margin-top: 0.5rem; }
    .retry-badge { background: rgba(127,119,221,0.15); border: 1px solid rgba(127,119,221,0.4); border-radius: 8px; padding: 0.4rem 0.8rem; font-size: 0.8rem; color: #7F77DD; display: inline-block; margin-top: 0.3rem; margin-left: 0.5rem; }
</style>
...
def get_neo4j_driver():
    ...
    raise ConnectionError(
        ...
        f"Please check NEO4J_URI, NEO4J_USERNAME, NEO4J_PASSWORD in secrets."
    )
...
def graph_retriever(driver, question):
    with driver.session() as session:
        # FIX: lowered word length filter from 3 to 2 to catch "AI", "LLM" etc
...
# ─────────────────────────────────────────
# FIX: Less strict critic agent
# ─────────────────────────────────────────
def critic_agent(graph_context, vector_context, answer):
    critic_llm = ChatGroq(model="llama-3.1-8b-instant", temperature=0)

    prompt = ChatPromptTemplate.from_template("""
You are a fact-checker. Check if the answer is mostly supported by the context.

CONTEXT (Knowledge Graph + Documents):
{context}
...
Rules:
- If the answer is mostly based on the context → reply: GROUNDED
- Only reply HALLUCINATED if the answer contains completely made-up facts
  not present anywhere in the context at all.

Reply with ONE word only: GROUNDED or HALLUCINATED
""")
...
def create_question_graph(driver, question):
    try:
        ...
        net = Network(height="500px", width="100%", bgcolor="#111128", font_color="#ffffff", directed=True)
        ...
        for record in all_data:
            ...
            if source not in nodes_added:
                net.add_node(source, label=source,
                    color={"background": colors.get(source_type, "#7F77DD"), "border": "#ffffff",
            ...
            if target not in nodes_added:
                net.add_node(target, label=target,
                    color={"background": colors.get(target_type, "#7F77DD"), "border": "#ffffff",
                           "highlight": {"background": "#EF9F27", "border": "#ffffff"}},
                    size=35, font={"size": 14, "color": "#ffffff", "bold": True},
        ...
        net.set_options("""{"physics":{"enabled":true,"barnesHut":{"gravitationalConstant":-5000,"centralGravity":0.5,"springLength":180,"springConstant":0.05,"damping":0.09},"stabilization":{"enabled":true,"iterations":200,"updateInterval":25}},"interaction":{"hover":true,"tooltipDelay":100,"zoomView":true,"dragView":true},"nodes":{"shape":"dot","shadow":true},"edges":{"shadow":true,"smooth":true}}""")
...
with st.sidebar:
    ...
    for tech in ["LangChain", "Neo4j AuraDB", "Groq LLaMA 3.1", "Groq Critic Agent", "ChromaDB", "Streamlit"]:
...
if not st.session_state.messages:
    ...
    <div style='text-align:center; color:#555; padding:2rem;'>
        ...
        <div style='font-size:1.1rem; color:#7F77DD; font-weight:600;'>Welcome to Self-Healing Graph RAG</div>
        <div style='font-size:0.85rem; color:#555; margin-top:0.3rem;'>Ask anything about AI agents — answers are critic-verified!</div>
    </div>
    """, unsafe_allow_html=True)
...
if question := st.chat_input("✨ Ask anything about AI agents..."):
    ...
    with st.chat_message("assistant"):
        ...
        with badge_col1:
            if verdict == "GROUNDED":
                st.markdown('<div class="verdict-grounded">✅ Critic verified: answer is grounded in sources</div>', unsafe_allow_html=True)
            else:
                st.markdown('<div class="verdict-hallucinated">⚠️ Could not fully verify — showing best available answer</div>', unsafe_allow_html=True)
        with badge_col2:
            st.markdown(f'<div class="retry-badge">🔄 Attempts: {attempts}</div>', unsafe_allow_html=True)
        ...
        with st.expander("📊 View Sources & Pipeline Debug"):
            ...
            with c2:
                ...
                st.markdown(f'<div class="pipeline-step">⚡ Verdict: {verdict} | Attempts: {attempts}/2</div>', unsafe_allow_html=True)
                if attempts > 1:
